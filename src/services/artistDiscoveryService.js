...
class ArtistDiscoveryService {
  ...
  /**
   * Follow an artist for a user
   */
  async followArtist(userId, artistData, notificationSettings = {}) {
    try {
      console.log(`➕ User ${userId} following artist: ${artistData.name}`);

      // First, ensure the artist exists in our database
      let artist = await Artist.findOne({ artistId: artistData.artistId });
      
      if (!artist) {
        // Create new artist record
        artist = await this.createArtistRecord(artistData);
      }
      ...
      const existingFollow = user.artistPreferences?.followedArtists?.find(
        follow => follow.artistId === artistData.artistId
      );

      if (existingFollow) {
        // Update notification settings
        existingFollow.notificationSettings = {
          ...existingFollow.notificationSettings,
          ...notificationSettings
        };
      } else {
        // Add new follow
        if (!user.artistPreferences) user.artistPreferences = { followedArtists: [] };
        ...
        user.artistPreferences.followedArtists.push({
          ...
          artistSpotifyId: artistData.externalIds?.spotifyId,
          followedAt: new Date(),
          notificationSettings: {
            releases: true,
            news: true,
        ...
        if (user.analytics?.listeningStats) {
          user.analytics.listeningStats.totalArtistsFollowed += 1;
        }
      }
      ...
      if (artist.analytics) {
        artist.analytics.followersCount = (artist.analytics.followersCount || 0) + 1;
        await artist.save();
      }

      // Store in RAG memory for personalization
      await ragMemoryService.storeMemory(userId, {
        type: 'preference',
        content: `User started following ${artistData.name} (${artistData.genres?.join(', ')})`,
        importance: 0.7,
        metadata: {
      ...
      console.log(`✅ Successfully followed ${artistData.name}`);
      return { success: true, artist, following: true };
  ...
  async unfollowArtist(userId, artistId) {
    try {
      console.log(`➖ User ${userId} unfollowing artist: ${artistId}`);
      ...
      const originalLength = user.artistPreferences?.followedArtists?.length || 0;
      if (user.artistPreferences?.followedArtists) {
        user.artistPreferences.followedArtists = user.artistPreferences.followedArtists.filter(
          follow => follow.artistId !== artistId
        );

        // Update analytics if artist was removed
        if (user.artistPreferences.followedArtists.length < originalLength) {
          if (user.analytics?.listeningStats) {
            user.analytics.listeningStats.totalArtistsFollowed = 
              Math.max(0, user.analytics.listeningStats.totalArtistsFollowed - 1);
          }
        }
      }
      ...
      if (artist?.analytics) {
        artist.analytics.followersCount = Math.max(0, (artist.analytics.followersCount || 0) - 1);
        await artist.save();
      }

      console.log(`✅ Successfully unfollowed ${artistId}`);
   ...
   */
  async getFollowedArtists(userId, options = {}) {
    try {
      const { includeDetails = true, sortBy = 'followedAt', limit } = options;
      ...
      let followedArtists = user.artistPreferences?.followedArtists || [];

      // Sort artists
      if (sortBy === 'followedAt') {
        followedArtists.sort((a, b) => new Date(b.followedAt) - new Date(a.followedAt));
      } else if (sortBy === 'name') {
        followedArtists.sort((a, b) => a.artistName.localeCompare(b.artistName));
      } else if (sortBy === 'priority') {
        const priorityOrder = { high: 3, medium: 2, low: 1 };
        followedArtists.sort((a, b) => priorityOrder[b.priority] - priorityOrder[a.priority]);
      }

      // Apply limit if specified
      if (limit) {
        followedArtists = followedArtists.slice(0, limit);
      }

      // Include detailed artist information if requested
      if (includeDetails) {
        const artistDetails = await Promise.all(
          followedArtists.map(async (follow) => {
            const artist = await Artist.findOne({ artistId: follow.artistId });
            return {
              ...follow.toObject(),
              details: artist || null
            };
