...
const testContent = {
  simple: 'Hello world',
  withTask:
    'Hello TASK_INFERENCE: {"taskType": "schedule", "parameters": {"when": "tomorrow"}} world',
  withEmotion:
    'Hello EMOTION_LOG: {"emotion": "happy", "intensity": 8, "context": "good news"} world',
  withBoth:
    'Hello TASK_INFERENCE: {"taskType": "reminder", "parameters": {"time": "5pm"}} and EMOTION_LOG: {"emotion": "excited", "intensity": 9, "context": "upcoming event"} world',
  complex:
    'This is a complex response with TASK_INFERENCE: {"taskType": "analyze", "parameters": {"data": "user_behavior", "timeframe": "last_week"}} and multiple EMOTION_LOG: {"emotion": "anxious", "intensity": 6, "context": "work stress"} markers with additional text and formatting.',
  messy:
    'TASK_INFERENCE: {"taskType": "test"} EMOTION_LOG: {"emotion": "happy"} some text TASK_INFERENCE: {"taskType": "another"} more text EMOTION_LOG: {"emotion": "sad"} final text'
};
...
const testMemoryUsage = () => {
  ...
  for (let i = 0; i < operations; i++) {
    ...
    if (i % 10000 === 0) {
      ...
      console.log(
        `  Operation ${i}: ${Math.round(currentMemory.heapUsed / 1024 / 1024)}MB (+${memoryDelta.toFixed(2)}MB)`
      );
    }
  }
  ...
  console.log(
    `Final memory usage: ${Math.round(finalMemory.heapUsed / 1024 / 1024)}MB (+${memoryDelta.toFixed(2)}MB)`
  );
...
const runTests = async () => {
  try {
    ...
    const sortedRegex = Object.entries(regexResults).sort(
      (a, b) => b[1].opsPerSec - a[1].opsPerSec
    );
    ...
    if (parseFloat(memoryResults.memoryDelta) > 50) {
      console.log(
        '⚠️  Memory usage is high. Consider implementing more aggressive garbage collection.'
      );
    }

    const avgOpsPerSec =
      Object.values(regexResults).reduce((sum, r) => sum + r.opsPerSec, 0) /
      Object.keys(regexResults).length;
    ...
    if (parseFloat(bufferResults.avgTime) > 0.1) {
      console.log(
        '⚠️  Buffer operations are slow. Consider increasing buffer size or optimizing regex patterns.'
      );
    }
...
export { performanceTest, testRegexPerformance, testMemoryUsage, testBufferPerformance };
