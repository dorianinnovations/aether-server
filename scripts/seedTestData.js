...
const emotions = [
  'happy', 'excited', 'grateful', 'content', 'optimistic', 'peaceful',
  'sad', 'anxious', 'frustrated', 'overwhelmed', 'tired', 'lonely',
  'neutral', 'curious', 'motivated', 'confident', 'relaxed', 'energetic'
];
  ...
  for (let i = 1; i <= 5; i++) {
    const user = new User({
      ...
      profile: new Map([
        ...
        ['interests', ['meditation', 'fitness', 'reading', 'music', 'travel'].slice(0, Math.floor(Math.random() * 3) + 1).join(', ')]
      ]),
      emotionalLog: []
    });
    ...
    for (let day = daysBack; day >= 0; day--) {
      ...
      for (let log = 0; log < logsPerDay; log++) {
        const logDate = new Date(now.getTime() - (day * 24 * 60 * 60 * 1000) + (log * 8 * 60 * 60 * 1000));
      ...
      for (let msg = 0; msg < messagesCount; msg++) {
        ...
        const memory = new ShortTermMemory({
          ...
          content: isUserMessage 
            ? (msg === 0 ? conversationStarters[Math.floor(Math.random() * conversationStarters.length)] 
...
async function testEndpoints() {
  ...
  for (const user of users) {
    ...
    // Test growth summary
    try {
      const response = await fetch(`http://localhost:5000/personal-insights/growth-summary?timeframe=week`, {
        headers: {
      ...
      if (response.ok) {
        ...
        console.log(`✅ Growth Summary: ${data.data.metrics.emotionalLogs} logs, ${data.data.metrics.positivityRatio}% positive`);
      } else {
